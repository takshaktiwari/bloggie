...
<div class="br-pagebody mg-t-5 pd-x-30">
    <form action="{{ url('admin/page/create') }}" method="POST" class="bg-white p-4 border shadow-sm" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" class="form-control" id="post_title" value="{{ $page->title }}" required="" >
        </div>
        <div class="row">
            <div class="col-md-8">
                <div class="row">
            ...
            <div class="col-md-4">
                ...
                <select name="status" class="form-control" required="">
                    ...
                    <option value="0" {{ selected($page->status, '0') }} >In-Active</option>
                </select>
            </div>
        </div>
        <div class="form-group">
            <textarea name="content" rows="5" class="form-control text-editor">{{ $page->content }}</textarea>
        </div>
        <div class="form-group">
            <label for="">Meta Title</label>
            <textarea name="m_title" rows="2" class="form-control" placeholder="Write Meta Title of product" maxlength="250">{{ $page->m_title }}</textarea>
        </div>
        <div class="form-group">
            <label for="">Meta Keywords </label>
            <textarea name="m_keywords" rows="2" class="form-control meta_field"  placeholder="Enter Meta keywords">{{ $page->m_keywords }}</textarea>
        </div>
        <div class="form-group">
            <label for="">Meta Description </label>
            <textarea name="m_description" rows="2" class="form-control meta_field" placeholder="Enter Meta description">{{ $page->m_description }}</textarea>
        </div>
